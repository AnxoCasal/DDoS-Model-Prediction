...
def main_raw(spark, raw_dir, temp_dir, file_writer, file_name='raw'):

    raw_paths = FileSystemHandler.scan_directory('./archivos/downloaded', 'csv')

    df = raw.merge_csv_files(spark, raw_paths)
    df = raw.stratify_dataframe(df)
    df = raw.refactor_headers(df)

    raw_parquet_dir = f'{raw_dir}/{file_name}.parquet'
...
def main_staging(df, staging_dir, temp_dir, file_writer, file_name='staging'):
    ...
    staging_parquet_dir = f'{staging_dir}/{file_name}.parquet'
...
def main_bussiness (df, temp_dir, business_dir, ports_dict, file_writer, file_name='business'):
    ...
    staging_parquet_dir = f'{business_dir}/{file_name}.parquet'
...
df = main_raw(spark, RAW_DIR, TEMP_DIR, file_writer)
